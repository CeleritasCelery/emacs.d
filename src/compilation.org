...
#+BEGIN_SRC emacs-lisp
  (defhydra bman-cmd (:exit t :columns 2)
    ...
    ("r" ($run-bman "-s emu -s sgdft -s sglp -s vclp -s lintra_ol -s FLG -s hip_listgen -s lintra" t) "RTL only")
...
#+BEGIN_SRC emacs-lisp
  (defun $run-simregress (file)
    "run simgress command for specified list"
    (interactive `(,(let ((reg-dir (f-join ($model-root) "verif/reglist/")))
                      (completing-read
                       "Select test list: "
                       (cons "other..."
                             (seq-sort-by 'length '<
    ...
    (let* ((model-root ($model-root))
           ...
           (test-list (cond ((equal file "other...") (read-file-name "Select test list: "))
                            ...
                            ((stringp file) (f-join model-root "verif/reglist" file))
      ...
      ($temp-link-git-worktree)
      ($compilation-start (format "simregress -net %s -l %s -C 'SLES11SP4&&20G' -save" ($get-model-arguments) test-list)
                          'compilation-mode)))
#+END_SRC
  ...
  root"
    (cl-some (lambda (proc)
               (let ((buffer (process-buffer proc))
                     (dependancies (alist-get mode $compilation-model-dependancies)))
                 (and (process-live-p proc)
                      (cl-some (lambda (m)
                                 (eq m (buffer-local-value 'major-mode buffer)))
                               dependancies)
