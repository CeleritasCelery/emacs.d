...
#+BEGIN_SRC elisp
  ...
  (defun $ivy-yank (x &optional list)
    (thread-first (if list list (list x))
      (string-join "\n")
      (kill-new)))

  (defun $ivy-yank-file-name (x &optional list)
    (thread-first '$correct-file-path
      (mapcar (if list list (list x)))
      (string-join "\n")
      (kill-new)))
...
#+BEGIN_SRC elisp
  (use-package counsel
    ...
    (setq counsel-git-cmd "/usr/intel/bin/git ls-files --full-name --"
          ...
          counsel-rg-base-command "rg --with-filename --no-heading --line-number --search-zip --color never %s"
...
#+BEGIN_SRC elisp
  ...
  (defun $shell-mode-hook ()
    ...
    (setq-local comint-prompt-regexp ($rx bol (or "╰─→" (: symbol "(" nums ")%") (: symbol ">")) " "))
