...
based on the discussion [[http://bling.github.io/blog/2016/01/18/why-are-you-changing-gc-cons-threshold/][here]] this is good way to prevent freezing but
also get good performance during large operations
#+BEGIN_SRC emacs-lisp
  ...
  the full power of helm"
    ...
    (let ((file ($get-path-at-point)))
      (when-let ((root (and (string-match-p (rx "$" (opt "{") "MODEL_ROOT" ) file)
        ...
        (setenv "MODEL_ROOT" (string-remove-suffix "/" (file-truename root))))
      (require 'helm-files)
  ...
  (use-package compile
    ...
    (setq compilation-buffer-name-function
          (defun $compilation-buffer-name (mode)
            (format "*%s %s*" (file-name-nondirectory (vc-git-root default-directory)) (downcase mode))))
...
#+BEGIN_SRC emacs-lisp
  ...
  (with-eval-after-load 'compile
    (dolist (regex `((ipgen-gmake ,(err-rx bol "gmake: *** [" (group-n 1 (>= 10 any)) "] Error " num)
                                  1)
                     (core-assembler ,(err-rx bol "Information: script '" filename
                                              "'\n" spc+ "stopped at line " line ->)
                                     1 2)
                     (connection-error ,(err-rx bol info "Error: " (group-n 3 ->)
                                                "\n" info "-" (or "E" "F") "-: [CRT-" nums "] Error in "
                                                -> " file " filename spc*
                                                "\n" info " Error at line# " line ->)
                                       1 2 nil nil nil (3 compilation-info-face))
                     (bman-verilog ,(err-rx bol "-I-:Error-" ->
                                            "\n-I-:" filename ", " line)
                                   1 2)
                     (ipgen-hdl-copy ,(err-rx bol "-I-:Error-[MPD] Module previously declared"
                                              "\n-I-:  The module was previously declared at: "
                                              "\n-I-:  \"" (group fill) "/" fill (or ".vs" ".sv") "\",")
                                     (1 "%s/dft_ipgen.hdl"))
                     (sgdft ,(err-rx bol "-E-:SGDFT" -> "FAILED"
                                     "\n-I-:  Error" ->
                                     "\n-I-:  Use" ->
                                     "\n-I-:Report: " filename)
                            1)
                     (simregress ,(err-rx spc+ "simregress invocation failed on " ->
                                          "\n" spc+ "Refer to " -> ": " filename)
                                 1)
                     (upf ,(err-rx bol "Error-[" (group-n 3 ->) "]" ->
                                   "\n" filename ", " line)
                          1 2 nil nil nil (3 font-lock-warning-face))
                     (emu ,(err-rx bol "-E-:FAILED: emubuild" -> "REASON : failed LOG :  "
                                   filename)
                          1)
                     (vcs-log ,(err-rx bol "  Log: " filename)
                              1)
                     (upfsem ,(err-rx bol "UPFSEM_4" spc+ (1+ word) spc+
                                      filename spc+ line)
                             1 2)
                     (fail-open ,(err-rx bol "Errormessage   : Failed to open input file ["
                                         filename "]")
                                1)
                     (json ,(err-rx bol "-E- Unable to parse JSON file " filename ":"
                                    -> "line " line
                                    " offset " col)
                           1 2 3)
                     (at-file ,(err-rx bol info "  Error occurred at File: " filename " Line: " line)
                              1 2)
                     ))
      ;; -I-:  Error occurred at File: /p/hdk/rtl/proj_tools/buildman/master/1.00.10/stages//vc_common.pm Line: 245

      ;; -F-: [CRT-023] Error in conncection file adhoc connection file /nfs/pdx/disks/sdg74_1172/tjhinckl/workspace/mdf-xtal-con/tools/ipgen/mdf_s_h_0/collage_work/dft/gen_subsystem/manual_network/collage/adhoc_connection.txt
      ;; Error at line# 78 - "ubpc_x12clk_dfxact_xtalclk_pulstrech/clk x12clk_mucc_trig_clkstop_rpt_0_9/clk" : Illegal line  "ubpc_x12clk_dfxact_xtalclk_pulstrech/clk x12clk_mucc_trig_clkstop_rpt_0_9/clk" in the adhoc connection file /nfs/pdx/disks/sdg74_1172/tjhinckl/workspace/mdf-xtal-con/tools/ipgen/mdf_s_h_0/collage_work/dft/gen_subsystem/manual_network/collage/adhoc_connection.txt

      ;; -I-:-E-:mdf_s_v_0: invalid HSD waiver: 'hsdid' attribute is missing or invalid in IP_tjhinckl_1537547355, file '/nfs/pdx/disks/sdg74_1172/tjhinckl/workspace/mdf-ip-uprev/tools/lint/waivers/tmp/mdf_mdfc_h_tmp_waivers_w.xml'

      ;; 537683: OVM_ERROR /p/hdk/rtl/cad/x86-64_linux26/dteg/itpp_reader/0.7.0/src/tlm_cmd_pipe_tap.sv:559 @ 29140000000 (dft_itpp_test.env.i_dft_common_val_env.itpp_sqr.i_tlm_cmd_pipe_tap0) [TAP0_ITPP_READER_TLM_ERROR] Capture fifo op 'greater' on line '352' - FAIL

      ;; -I-:ERROR: mdf_s_v_0 has 1 upf error
      ;; -I-:Check: /nfs/pdx/disks/sdg74_1172/tjhinckl/workspace/mdf-turnin/target/mdf_10nm/lp/vclp/mdf_s_v_0/mdf_s_v_0/reports/uncompressed.rpt

      ;; -I-:Detailed violation log for sgdft_drc for mdf_s_v_0: /nfs/pdx/disks/sdg74_1172/tjhinckl/workspace/mdf-turnin/target/mdf_10nm/SG/sgdft/mdf_s_v_0/mdf_s_v_0/logs/sgdft_drc.log

      ;; -E-:Error in stage bman.mdf_10nm.vclp.vclp_mdf_s_v_0:
      ;; -I-:***** Cat'ing logs *****
      ;; -I-:Executing: cat /nfs/pdx/disks/sdg74_1172/tjhinckl/workspace/mdf-afd/target/mdf_10nm/lp/vclp/mdf_s_v_0/mdf_s_v_0/vclp.log
...
#+BEGIN_SRC emacs-lisp
  ...
  (defvar bman-error-regexp-alist-alist
    `((stage ,(err-rx bol "-" (or "E" "I") "-:" (opt spc) "FAILED:"
                      -> (or ";" ":") " LOG " (opt ": ") filename)
             1)
      (generic-stage ,(err-rx bol "-E-:FAILED" spc+ fp
                              spc+ fp "  "
                              filename)
  ...
  (defvar $compilation-font-lock-keywords
    `((,(log-rx I: spc* (group (in "#*") (in " \t#*=") ->) eol)
      ...
      (,(log-rx I: (group (or "-I-" "Note-" "NOTE" "OVM_INFO")))
       1 'compilation-info prepend)
      (,(log-rx I: (group (or "-E-" "-F-" "Error-" "Fatal-" "OVM_ERROR" "ERROR" "OVM_FATAL")))
