...
use these function to get the cluster therfore the model needed. At
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun $remove-file-async (file)
    (when (file-exists-p file)
      (async-start
       (lambda ()
         (shell-command (format "mv %s %s.del && rm -rf %s.del 2>/dev/null" file)))
       (lambda (result)
         (message "async delete done")))))
    ...
    (let* ((model-root (file-truename (vc-git-root default-directory)))
           ...
           (compilation-buffer-name-function
            (lambda (_mode)
              (format "*%s bman*"
                      (f-filename model-root)))))
      ($compilation-start (concat (when (and (file-exists-p "target")
                                             (or current-prefix-arg clean))
                                    "mv target target.del && (rm -rf target.del 2>/dev/null &) && ")
                                  "source /p/hdk/rtl/hdk.rc -cfg shdk74 && bman" ($get-model-arguments) filter)
                          'bman-mode)))
