...
*** general
We need to set =evil-want-integration= before loading evil so that
=evil-collection= can overide the modes. the very magic setting will
make the evil search regex more PCRE compatible.
#+BEGIN_SRC emacs-lisp
  (use-package evil
    :demand t
    :init
...
*** functions
functions that are used for compilation of ipgen and bman. Need to
make the bman function more robust and flexible
