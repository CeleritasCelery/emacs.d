        ...
        (setq $compilation-error-regexp-alist
              `((,(err-rx ^ "-I-:Error-" ->
                ...
                (,(err-rx ^ "-E-:Error in stage bman.mdf_10nm.vclp." symbol ":"
                 ...
                 1)
                (,(err-rx ^ "-I-:DIE signal: ERROR 103: Couldn't find directory '" filename "' on path:")
                 1)
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-compilation-mode ipgen-mode "ipgen"
    ...
    (add-hook 'compilation-filter-hook '$compilation-apply-ansi-color nil 'local))
#+END_SRC
    ...
    (let* ((model-root (file-truename (vc-git-root default-directory)))
           ...
           (compilation-buffer-name-function
            (lambda (_mode)
              (format "*%s bman*"
                      (f-filename model-root)))))
      (compilation-start (concat (when current-prefix-arg "mv target target.del && (rm -rf target.del 2>/dev/null &) && ")
                                 "source /p/hdk/rtl/hdk.rc -cfg shdk74 && bman -dut mdf_10nm " filter)
                         'bman-mode)))
