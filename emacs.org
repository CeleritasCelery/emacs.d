...
Define use-package for org so that we get the correct version
#+begin_src emacs-lisp
  (use-package org)
#+end_src
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'midnight-hook (lambda () (mu4e-update-mail-and-index t)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package org-roam
    :init
    (setq org-roam-directory "/home/foco/repos/logseq"
          ...
          '(("d" "default" entry
             #'org-roam-capture--get-point "* %?"
             :file-name "journals/%<%Y_%m_%d>" :head "#+title: %<%b %o, %Y>\n")))
...
#+begin_src emacs-lisp
  ...
  (advice-add #'rustic-popup :after #'evil-emacs-state)
